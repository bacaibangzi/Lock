 ...
 */
(function($) {
	var f_height = document.documentElement.clientHeight ;
	//alert(f_height);
    ...
    var settings = {
        ...
        iframeHeight: f_height, //固定TAB中IFRAME高度,根据需要自己修改
        target: '.nav-tabs',
    ...
    $.addtabs.add = function(opts) {
        ...
        if (tab_li.find('#tab_' + id).length < 1) {
            ...
            var title = $('<li>', {
                ...
                'role': 'tab',
                'data-toggle': 'tab'
            }).html(opts.title +"<i class='close-tab glyphicon glyphicon-remove'></i>"));
           // alert(opts.title);
            //是否允许关闭
            /*
            if (settings.close) {
