...
<title>角色修改</title>
	...
	<body>
		<div  class="form-mod"><div style="color:#F00;font-size:22px;">${vo.errMsg}</div>
			<form:form commandName="form" id="form" action="../yonghu/save.htm" method="post">
				...
				<input type="hidden" name="uiLoginName" value="${form.uiLoginName }" />
				<input type="hidden" name="uiPassword" value="${form.uiPassword }" />
				...
				<tbody>
					...
					<tr>	
						...
						<td >
							<form:input path="uiNum"  cssClass="input-text {required: true, maxlength: 20}"/>
						</td>
					</tr>
					<tr>	
						<td class="hd" >
					...
					<tr>	
						...
						<td >
							<form:select path="uiType" items="${typeMap}"  cssClass="input-select {required: true}"/> 
						</td>
					</tr>
					<tr>	
						<td class="hd" >
						...
						<td >
							<input name="uiVipLevelTime"  id="uiVipLevelTime"  class="input-text {maxlength: 20}" value="${form.uiVipLevelTime}"
								readonly="readonly"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" 
								onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" cssClass="input-text { maxlength: 20}"/>
						</td>
					</tr>
					<tr>	
						<td class="hd" >
					...
					<tr>	
						...
						<td >
							<form:input path="uiAddress"  cssClass="input-text {maxlength: 60}"/>
						</td>
					</tr>
					<tr>	
						<td class="hd" >
						...
						<td >
							<form:input path="uiVipLevelName"  cssClass="input-text {maxlength: 60}"/>
						</td>
					</tr>
					<tr>	
						<td class="hd" >
						...
						<td >
							<input name="uiFindPasswordTime"  id="uiFindPasswordTime"  class="input-text {maxlength: 20}" value="${form.uiFindPasswordTime}"
								readonly="readonly"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" 
								onfocus="WdatePicker({dateFmt:'yyyy-MM-dd})" cssClass="input-text { maxlength: 20}"/>
						</td>
					</tr>
		...
		<script type="text/javascript">
		    ...
		    function submitCheck(){
		    	/*
				if($("#uiVipLevelTime").val()==''){
					alert("出生日期不能为空");
					return;
				}
				if($("#uiFindPasswordTime").val()==''){
					alert("入学日期不能为空");
					return;
				}
				*/
